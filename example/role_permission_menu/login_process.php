...
ดังนั้น เมื่อแฟ้ม 'จัดการบทความ' ตัวจริงใน $itemsById ถูกแก้ไข (โดยการที่ลูกๆ ของมันวิ่งเข้ามาหา) แฟ้มที่อยู่ในตู้ $structuredMenu ซึ่งเป็นแค่ "ทางเชื่อม" ก็จะเห็นการเปลี่ยนแปลงนั้นไปด้วยโดยอัตโนมัติ
นี่คือหลักการที่ทรงพลังของ Loop นี้ครับ มันใช้ Array เดียว ($itemsById) เป็นทั้ง "กองแฟ้มที่รอจัด" และ "พื้นที่ทำงาน" ไปในตัว เพื่อสร้างโครงสร้างต้นไม้ที่สมบูรณ์ขึ้นมา แล้วผลลัพธ์สุดท้ายก็คือ $structuredMenu ที่ชี้ไปยังแฟ้มหลักที่ถูกจัดเรียงเรียบร้อยแล้วนั่นเองครับ
สรุปก็คือ
...
/*
วิธีที่ 1: เก็บใน Session (แบบปัจจุบันที่เราทำอยู่)
หลักการทำงาน: คำนวณและสร้าง Array เมนูที่ซับซ้อน เพียงครั้งเดียว ตอนที่ผู้ใช้ล็อกอินสำเร็จ 
จากนั้นเก็บ Array นั้นไว้ใน $_SESSION['user_menu'] 
ทุกๆ หน้าที่ผู้ใช้เปิดหลังจากนั้น ก็แค่ดึง Array ที่สร้างเสร็จแล้วจาก Session มาใช้งานได้เลย ไม่ต้องคุยกับฐานข้อมูลอีก

วิธีที่ 2: ไม่เก็บใน Session (สร้างใหม่ทุกครั้ง)
หลักการทำงาน: ตอนล็อกอิน จะเก็บแค่ข้อมูลพื้นฐานของผู้ใช้ เช่น user_id, role_id ไว้ใน Session 
แต่ จะไม่เก็บ Array เมนู ในทุกๆ ครั้งที่ผู้ใช้เปิดหน้าใหม่ (เช่น dashboard.php, profile.php) 
จะต้องเรียกฟังก์ชัน buildMenuForRole() เพื่อคุยกับฐานข้อมูลและสร้าง Array เมนูขึ้นมาใหม่สดๆ ทุกครั้ง
*/