...
class DocumentService
{
    ...
    public function managePoAndInspection(array $poData): bool
    {
        ...
        try {
            ...
            // $poId = $poData['headerData']['po_id'] ?? 0;
            
            // 1. ตรวจสอบและจัดการข้อมูล Header (INSERT หรือ UPDATE)
            // <begin>public function save(array $poData):int(คือค่า po_id)
            ...
            // }
            // </begin>
            $poId = $this->po->save($poData['headerData']);
$_SESSION['data in document service XXXXXXXXXXXXXXXXXXXx']=$poData['headerData'];
            ...
            // $deleteItems = array_filter($poData['$periodsData'], fn($item) => ($item['period_crud'] ?? 'none') === 'delete');
            // $updateItems = array_filter($poData['$periodsData'], fn($item) => ($item['period_crud'] ?? 'none') === 'update');
            // $createItems = array_filter($poData['$periodsData'], fn($item) => ($item['period_crud'] ?? 'none') === 'create');
